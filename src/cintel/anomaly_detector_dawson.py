"""
anomaly_detector_dawson.py - Project script (example).
...
Paths (relative to repo root)
    ...
    INPUT FILE: data/clinic_data_dawson.csv
    OUTPUT FILE: artifacts/anomalies_dawson.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.anomaly_detector_dawson
...
DATA_FILE: Final[Path] = DATA_DIR / "clinic_data_dawson.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "anomalies_dawson.csv"
...
def main() -> None:
    ...
    # y is height in inches, so 15 inches is the lower limit (unusually short)
    MIN_REASONABLE_Y_VALUE: Final[float] = 15.0
    ...
    # Add a reason column explaining which value(s) triggered the anomaly.
    anomalies_df = anomalies_df.with_columns(
        pl.when(
            (pl.col("age_years") >= MAX_REASONABLE_X_VALUE)
            & (pl.col("height_inches") >= MAX_REASONABLE_Y_VALUE)
        )
        .then(pl.lit("age too high; height too high"))
        .when(
            (pl.col("age_years") >= MAX_REASONABLE_X_VALUE)
            & (pl.col("height_inches") < MIN_REASONABLE_Y_VALUE)
        )
        .then(pl.lit("age too high; height too low"))
        ...
        .otherwise(pl.lit("height too low"))
        .alias("reason")
    )
